...
const broadcastToTable = (tableId: string, data: any) => {
  ...
  for (const client of connectedClients) {
    if (
      client.role === 'table' &&
      (client.tableId === tableId || tableId === 'all') &&
...
app.post('/api/menu', (req, res) => {
  const { name, description, price, category, image, tags, inStock, station } =
    req.body;
...
app.patch('/api/orders/:id', (req, res) => {
  ...
  if (acknowledgedByStaff !== undefined)
    order.acknowledgedByStaff = acknowledgedByStaff;
...
app.post('/api/orders/:id/request-bill', (req, res) => {
  ...
  broadcastToStaff({ type: 'order:update', order: orders[idx] });
  broadcastToStaff({ type: 'tables:update', tables });
  broadcastToTable(orders[idx].tableId, {
    type: 'order:update',
    order: orders[idx],
  });
...
app.post('/api/seed/external-api', async (req, res) => {
  try {
      ...
      try {
        ...
        if (data && data.meals) {
          ...
          meals.forEach((meal: any) => {
            ...
            const price = 8.5 + (idNum % 17) + 0.5; // $9.00 to $25.50
      ...
      try {
        ...
        if (data && data.drinks) {
          ...
          drinks.forEach((drink: any) => {
            ...
            const price = 6.0 + (idNum % 7) + 0.5; // $6.50 to $13.50
            ...
            fetchedItems.push({
              ...
              tags: ['Craft Drink', drink.strAlcoholic || 'Cocktail'].filter(
                Boolean,
              ),
              inStock: true,
              station: 'bar',
        ...
        console.warn(
          `Failed to fetch drink term from TheCocktailDB: ${term}`,
          e,
        );
      }
    }
    ...
    if (fetchedItems.length > 0) {
      ...
      res.json({
        success: true,
        count: currentMenuItems.length,
        menu: currentMenuItems,
      });
    } else {
      res
        .status(500)
        .json({ error: 'Failed to fetch external menu items from APIs.' });
    }
  } catch (error: any) {
...
app.post('/api/assistance/request', (req, res) => {
  ...
  if (!tableId || !requestType) {
    return res
      .status(400)
      .json({ error: 'tableId and requestType are required' });
  }
...
const startApp = async () => {
  if (process.env.NODE_ENV !== 'production') {
    ...
    console.log(
      'Starting server in PRODUCTION mode serving static dist files...',
    );
